...
import transform
from my_dataset import VOC2012DataSet, HANDDataSet
from src.ssd_model import SSD300, Backbone
import train_utils.train_eval_utils as utils
from train_utils.coco_utils import get_coco_api_from_dataset


def create_model(num_classes=26, device=torch.device('cpu')):
...
def main(parser_data):
    ...
    # check voc root
    if os.path.exists(os.path.join(VOC_root, "HAND")) is False:
        raise FileNotFoundError("HAND dose not in path:'{}'.".format(VOC_root))

    train_dataset = HANDDataSet(VOC_root, data_transform['train'], train_set='train.txt')
    ...
    val_dataset = HANDDataSet(VOC_root, data_transform['val'], train_set='val.txt')
    ...
    model = create_model(num_classes=26, device=device)
    model.to(device)
